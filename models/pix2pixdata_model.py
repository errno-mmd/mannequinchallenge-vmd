...
from models.networks import cuda_if_available
import sys
import h5py
import os.path
...
class Pix2PixDataModel(base_model.BaseModel):
    ...
    def forward(self):
        ...
        # run first network
        self.input_images = autograd.Variable(cuda_if_available(self.input), requires_grad=False)
        human_mask = 1.0 - autograd.Variable(
            cuda_if_available(self.targets['env_mask']), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            cuda_if_available(self.targets['keypoints_img']), requires_grad=False).unsqueeze(1)
        ...
        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                cuda_if_available(self.targets['input_log_depth']),
                requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                cuda_if_available(self.targets['input_confidence']),
                requires_grad=False).unsqueeze(1)
    ...
    def evlaute_M_error(self, input_, targets, n_iter, write_to_summary):
        ...
        input_imgs = autograd.Variable(cuda_if_available(input_), requires_grad=False)
        # stack inputs
        human_mask = 1.0 - autograd.Variable(
            cuda_if_available(targets['env_mask']), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            cuda_if_available(targets['keypoints_img']), requires_grad=False).unsqueeze(1)

        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                cuda_if_available(targets['input_log_depth']), requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                cuda_if_available(targets['input_confidence']), requires_grad=False).unsqueeze(1)
    ...
    def eval_save_tum_img(self, input_, targets, save_path):
        input_imgs = autograd.Variable(cuda_if_available(input_))
        human_mask = 1.0 - autograd.Variable(
            cuda_if_available(targets['env_mask']), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            cuda_if_available(targets['keypoints_img']), requires_grad=False).unsqueeze(1)
    ...
    def run_and_save_videos_prediction(self, input_, targets, save_path):
        ...
        mvs_depth = autograd.Variable(cuda_if_available(targets['mvs_depth']), requires_grad=False)
        input_depth = autograd.Variable(
            cuda_if_available(targets['input_depth']), requires_grad=False)
